...
Используйте один из четырех вариантов обучающего модуля.
...
Базовые кандидаты: `variable`, `value`, `data type`, `integer`, `floating-point number`, `string`, `Boolean`, `assignment`, `expression`, `operator`.

#### Вариант 2. Conditional Statements
`condition`, `Boolean expression`, `branch`, `if statement`, `else clause`, `comparison operator`, `nested condition`, `control flow`, `truth value`, `indentation`.

#### Вариант 3. Loops and Iteration
`loop`, `iteration`, `for loop`, `while loop`, `counter`, `range`, `break`, `continue`, `loop body`, `termination condition`.

#### Вариант 4. Functions and Parameters
`function`, `parameter`, `argument`, `return value`, `function call`, `scope`, `local variable`, `default argument`, `docstring`, `recursion`.

Для каждого варианта: 15+ терминов, TBX, XLSX для Smartcat, TMX с 8+ сегментами и 5 CAT-наблюдений.
