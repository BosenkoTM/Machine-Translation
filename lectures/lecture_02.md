...
$$
P(y \mid x)=\prod_{t=1}^{n} P\left(y_t \mid y_{1:t-1}, x\right).
$$

На каждом шаге система прогнозирует следующий токен с учётом исходного текста и уже сгенерированной части перевода.

Обучение по параллельному корпусу обычно связано с минимизацией отрицательного логарифмического правдоподобия:
...
где <i>y</i><sub>t</sub><sup>*</sup> — токен эталонного перевода на шаге <i>t</i>, а (<i>y</i><sub>1</sub><sup>*</sup>, …, <i>y</i><sub>t−1</sub><sup>*</sup>) — последовательность предшествующих токенов эталонного перевода.
...
Матрица весов внимания определяется как
...
Элемент <i>A</i><sub>ij</sub> матрицы <i>A</i> показывает, насколько сильно представление позиции <i>i</i> использует информацию позиции <i>j</i> на данном слое и в данной голове внимания.
...
Каждая голова внимания вычисляется независимо:
...
Использование нескольких голов позволяет модели параллельно учитывать различные зависимости между элементами последовательности. При этом методически некорректно утверждать, что каждая конкретная голова внимания всегда однозначно соответствует определённому синтаксическому или семантическому правилу.
...
При жадном декодировании на каждом шаге выбирается токен с максимальной условной вероятностью:
...
Однако локально оптимальный выбор токена не всегда приводит к наиболее вероятной или качественной полной последовательности. **Beam search** хранит несколько наиболее вероятных гипотез и развивает их параллельно, сравнивая накопленные оценки последовательностей.
